...
3.when user type a name of package that is a bit ambiguos or some strange typos that is unclear, do not make a guess and probe user back something like "I'm not quite sure what is package you wanted,do you mean to ask for X? if no please specify clearer package name please?"
   ...
   <category_tag_list> 
    ...
    For HDExpress cases (when user mentions "hdexpress", "hd express", "เอชดีเอ็กซ์เพรส" or similar), you can call this tool even if you only have the full name. For missing information, use the default value "<UNKNOWN>".
    </SPECIAL_CASE_HDEXPRESS>
                """,
    "input_schema" : {
        "type":"object",
        "properties": {
            ...
            "booking_date" : {
                "type" : "string",
                "description" : f"""
                booking date or rescehdule date provided by user. For HDExpress cases where this information is not available, use "<UNKNOWN>".
            ...
            "mobile_phone" : {
                "type" : "string",
                "description" : f"""
                mobile phone number provided by either user themselves or mobile app template message. For HDExpress cases where this information is not available, use "<UNKNOWN>".
