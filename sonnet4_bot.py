...
class CentralizedTools:
    ...
    def __init__(self, rag: RAG, tools: Tools):
        """Initialize with RAG and Tools instances"""
        self.rag = rag
        self.tools = tools
...
class JibAI:
    ...
    def __init__(self, global_storage):
        ...
        self.centralized_tools = CentralizedTools(self.rag, self.tools)
    ...
    def _load_system_prompt(self):
        ...
        try:
            with open('src/prompts/interpret.txt', 'r', encoding='utf-8') as f:
...
"""
    ...
    async def _url_to_base64(self, url: str) -> Tuple[bool, str, str]:
        """
        Convert image URL to base64 for Bedrock compatibility
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                logger.info(f"🔄 [URL-TO-BASE64] Converted image: {len(base64_data)} chars, type: {content_type}")
    ...
    async def forward(self, chats, room_id, last_query) -> Tuple[Dict, Dict, Dict]:
        ...
        try:
            ...
            try:
                ...
                # Ultimate fallback if handover generation also fails
                handover_response = "เกิดปัญหาทางเทคนิคค่ะ 🔧 ระบบจะโอนให้เจ้าหน้าที่มาช่วยเหลือทันทีนะคะ รบกวนรอสักครู่ค่า 🙏"
    ...
    async def _generate_handover_fallback(self, error_context: str = "") -> str:
        ...
        try:
            # Use the centralized handover tool
            handover_result = await self.centralized_tools.execute_tool(
                "handover_asap",
                {
                    "issue_description": f"Technical error requiring immediate assistance: {error_context}",
                    "reasoning": "System error detected - immediate handover to human agent"
                }
            )
            ...
            return "เกิดปัญหาทางเทคนิคค่ะ 🔧 ระบบจะโอนให้เจ้าหน้าที่มาช่วยเหลือทันทีนะคะ รบกวนรอสักครู่ค่า 🙏"
