...
def extract_attachment_urls(attachments: List[Dict]) -> List[str]:
    """Extract just the URLs from attachment fields"""
    return [attachment['url'] for attachment in attachments if 'url' in attachment]

def extract_attachment_info(attachments: List[Dict]) -> List[Dict]:
    """Extract useful info from attachments (filename, url, size)"""
    return [
        {
        ...
        for att in attachments if 'url' in att
    ]
...
async def _url_to_base64_content(url: str) -> Optional[Dict]:
    ...
    try:
        # Try to get smaller image version for Airtable URLs
        modified_url = url
        if 'airtableusercontent.com' in url:
            # Add size parameters for Airtable images to get much smaller version
            # Use more aggressive sizing to reduce token usage
            if '?' in url:
                modified_url = f"{url}&w=200&h=200&fit=crop&q=60"  # 200px + lower quality
            else:
                modified_url = f"{url}?w=200&h=200&fit=crop&q=60"  # 200px + lower quality
            
            print(f"🔧 [URL-DEBUG] Original: {url[:100]}...")
            print(f"🔧 [URL-DEBUG] Modified: {modified_url[:100]}...")
            print(f"🔧 [URL-DEBUG] Parameters added: w=200&h=200&fit=crop&q=60")
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            print(f"📊 [IMAGE-SIZE] File: {file_size_kb:.1f}KB, Base64: {len(image_data):,} chars")
